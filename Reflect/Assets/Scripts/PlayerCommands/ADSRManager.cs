...
// Used Professor's ADSRManager as a template
public class ADSRManager : MonoBehaviour
{
    ...
    private string RightInputKey = "d";
    private string LeftInputKey = "a";
    ...
    void Update()
    {
        if (Input.GetKeyDown(this.RightInputKey))
        {
            this.ResetTimers();
        ...
        if (Input.GetKey(this.RightInputKey))
        {
            // this.CurrentPhase = Phase.Sustain;
            this.InputDirection = 1.0f;
        }

        if (Input.GetKeyUp(this.RightInputKey))
        {
            this.InputDirection = 1.0f;
            this.CurrentPhase = Phase.Release;
        }

        if (Input.GetKeyDown(this.LeftInputKey))
        {
            this.ResetTimers();
            this.CurrentPhase = Phase.Attack;
            this.InputDirection = -1.0f;
        ...
        if (Input.GetKey(this.LeftInputKey))
        {
            // this.CurrentPhase = Phase.Sustain;
            this.InputDirection = -1.0f;
        }

        if (Input.GetKeyUp(this.LeftInputKey))
        {
            this.InputDirection = -1.0f;
            this.CurrentPhase = Phase.Release;
        }
    ...
    float ADSREnvelope()
    {
        ...
        {
            // Setting ReleaseTimer to 0 so that the button can be tapped
            // without triggering a switch to Phase.none
